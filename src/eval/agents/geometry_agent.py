...
# Type definitions
ActionDict = dict  # {'V0', 'phi', 'theta', 'a', 'b'}
BallsDict = dict  # {ball_id: Ball}
...
class GeometryAgent(Agent):
    ...
    def __init__(
        ...
        # print("GeometryAgent (Geometry-Guided) 已初始化。")

    # ============ 辅助方法 ============
    @staticmethod
    def check_remaining_targets(balls: BallsDict, my_targets: List[str]) -> Tuple[List[str], bool]:
        """检查剩余目标球并判断是否需要切换到黑8"""
    ...
    def decision(
        ...
        if balls is None or my_targets is None or table is None:
            print("GeometryAgent: 输入不完整，使用随机动作")
        ...
        try:
            ...
            if best_action is None or best_score < -50:
                # print(f"未找到好方案 (最高分: {best_score:.1f})，使用随机动作")
                return self._random_action()
